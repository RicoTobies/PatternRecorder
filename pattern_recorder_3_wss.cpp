...
public:
    ...
    void scaleToRangenum(vector<Note3>& newNotes, float startTime, float endTime, float startIndexnum, float endIndexnum, int schrittwanz, bool firstrec);
...
void PatternRecorder::convertNotesToRenoisePattern(int patternLength) {
    ...
    // Copy to clipboard
    if (SDL_SetClipboardText(patternXml.c_str()) != 0) {
        std::cerr << "Failed to set clipboard text: " << SDL_GetError() << std::endl;
    } else {
...
//         auto* instrumentElement = doc.NewElement("Instrument");
//         char instHex[3];
//         sprintf(instHex, "%02X", note.instrument);
//         instrumentElement->SetText("00");
//         noteColumn->InsertEndChild(instrumentElement);
//     }
...
//     // Copy to clipboard
//     SDL_SetClipboardText(patternXml.c_str());
//     std::cout << "Pattern copied to clipboard with " << patternLength << " lines" << std::endl;
// }
...
//         char instHex[3];
//         sprintf(instHex, "%02X", note.instrument);
//         instrumentElement->SetText(instHex);
//         noteColumn->InsertEndChild(instrumentElement);
//     }
...
void PatternRecorder::pasteRenoisePattern() {
    ...
    if (clipboardText) {
        std::string patternXml(clipboardText);
        SDL_free(clipboardText);
        
        // Convert the pattern
        convertRenoisePatternToNotes(patternXml);
    }
}
...
//         if (!note && !instrument && lineIndex == 0) {
//             notes4.push_back({0, 0, 0});
//         }
//         if (note && instrument) {
//             // Convert note string to MIDI number
//             std::string noteStr = note->GetText();
...
void PatternRecorder::DisPlNotes() {
    ...
    for (int i=0; i<notes3.size();++i) {
        cout << i << std::setw(10) << notes3[i].timestamp*sharedData->zeitfakto << ", " << notes3[i+1].timestamp*sharedData->zeitfakto-(notes3[i].timestamp*sharedData->zeitfakto) << ", " << notes3[i].instrument << ", " << notes3[i].pitch << ", " << notes3[i].p2 << endl;
    }
    cout << "notes4" << endl;
    for (int i=0; i<notes4.size();++i) {
        cout << i << std::setw(10) << notes4[i].timestamp*sharedData->zeitfakto << ", " << notes4[i+1].timestamp*sharedData->zeitfakto-(notes4[i].timestamp*sharedData->zeitfakto) << ", " << notes4[i].instrument << ", " << notes4[i].pitch << ", " << notes4[i].p2 << endl;
    }
    cout << "snotes" << endl;
    for (int i=0; i<snotes.size();++i) {
        cout << i << std::setw(10) << snotes[i].timestamp << ", " << snotes[i+1].instrument << notes4[i].pitch << endl;
    }
...
void PatternRecorder::record() {
    ...
    bool scaletoa = false;
    bool mschrittwanz = false;
    int numfactor = 1;
    float rschrittwanz =1.0f;  
    while (true) {
        SDL_Event event;
        int noteValue;
        int noteValue2;
        ...
        if (SDL_PollEvent(&event)) {
            ...
            if (settimestamp) {
                if (!recordingStarted) {
                    ...
                    notes.push_back({0, static_cast<int>(notes.size()), noteValue, scaletoa, mschrittwanz, rschrittwanz}); // Instrument ist die Anzahl der bisherigen Noten
                    ...
                    // Füge die Note zur Liste hinzu, Instrument ist die Anzahl der bisherigen Noten
                    notes.push_back({currentTimestamp, static_cast<int>(notes.size()), noteValue, scaletoa, mschrittwanz, rschrittwanz}); // Instrument ist die Anzahl der bisherigen Noten
                }
    ...
    for (const auto& note : notes) {
        cout << note.timestamp << ", " << note.instrument << ", " << note.pitch << ", " << note.scl << ", " << note.mschrittwanz << ", " << note.schrittwanz << endl;
    }
    int ie = notes.size();
    cout<<"ie="<<ie<<endl;
    for (int i=0; i<ie; ++i) {
            if(notes[i].scl==1 && notes[i+1].scl==1) {
                ...
                // schrittwanz=(float)(notes[i+2].pitch);
                ...
                // if (notes[i].mschrittwanz==false) {
                    rschrittwanz=(endIndexnum-startIndexnum)*(endIndexnum-startIndexnum)/(endIndexnum-startIndexnum);
                    scaleToRangenum(newNotes, startTime, endTime, startIndexnum, endIndexnum, rschrittwanz, firstrec);
                // }
                // else scaleToRangenum(newNotes, startTime, endTime, startIndexnum, endIndexnum, notes[i+1].schrittwanz, firstrec);
                notes[i+1].pitch=1;
                notes[i].scl=0;
                notes[i+1].scl=0;
    ...
    //         schrittwanz=(endIndexnum-startIndexnum)*(endIndexnum-startIndexnum)/(endIndexnum-startIndexnum);
    //         scaleToRangenum(newNotes, startTime, endTime, startIndexnum, endIndexnum, schrittwanz, firstrec, notestoscale);
    //     }
    // }
...
//     return radii;
// }
void PatternRecorder::scaleToRangenum(vector<Note3>& newNotes, float startTime, float endTime, float startIndexnum, float endIndexnum, int schrittwanz, bool firstrec) {
    ...
    cout<<"schrittwa"<<std::setw(15)<<"schrittwe"<<std::setw(15)<<"schrittwanz"<<endl;
    cout<<schrittwa<<std::setw(15)<<schrittwe<<std::setw(15)<<schrittwanz<<endl;
    cout<<endl;

if (!kk && !kl && !kr) {
    Indexnumlaenge = 1 - (1/startIndexnum);
    ...
    float kd = Indexnumlaenge-schrittwe*(float)(schrittwanz);
    int sumanz = schrittwanz*(schrittwanz+1)/2;
    float kdtanz = kd/sumanz;
    cout<<"kd: "<<std::setw(15)<<"sumanz: "<<std::setw(15)<<"kdtanz: "<<endl;
    cout<<kd<<std::setw(15)<<sumanz<<std::setw(15)<<kdtanz<<endl;
    cout<<endl;
    cout<<"i: "<<std::setw(15)<<" näschritt: "<<std::setw(15)<<" schrittwe+(schrittwanz-i+1)*kdtanz: "<<std::setw(15)<<" (schrittwanz-i+1)*kdtanz: "<<endl;
        for (float i=1; i<=schrittwanz;++i) {
                näschritt+=schrittwe+((schrittwanz-i+1)*kdtanz);
                cout<<i<<std::setw(15)<<näschritt<<std::setw(15)<<schrittwe+(schrittwanz-i+1)*kdtanz<<std::setw(15)<<(schrittwanz-i+1)*kdtanz<<endl;
                näschrittskl.push_back({i,näschritt});
        }
    }
    else {
    float kd = Indexnumlaenge-schrittwa*(float)(schrittwanz);
    int sumanz = schrittwanz*(schrittwanz+1)/2;
    float kdtanz = kd/sumanz;
    cout<<"kd: "<<std::setw(15)<<"sumanz: "<<std::setw(15)<<"kdtanz: "<<endl;
    cout<<kd<<std::setw(15)<<sumanz<<std::setw(15)<<kdtanz<<endl;
    ...
    Indexnumlaenge = 1 - (1/endIndexnum);
    ...
    for (float i=0;i<näschritts.size();++i){
        currentsteptimescl=totalDuration*näschritts[i].xn+startTime;
...
void PatternRecorder::vecforrecf() {
    ...
    for (int i = 0; i < recsteps; ++i) {
        ...
        cout << "Geben Sie die schrittwanz ein: "<<endl;
    ...
    for (int i=0;i<recsteps;++i) {
       ...
       schrittwanz = veforec[i].steps;
       startTime = notes4[i].timestamp;
       endTime = notes4[i+1].timestamp;
       cout<<"startIndex: "<<startIndex<<" endIndex: "<<endIndex<<" startIndexnum: "<<startIndexnum<<" endIndexnum: "<<endIndexnum<<" schrittwanz: "<<schrittwanz<<" startTime: "<<startTime<<" endTime: "<<endTime<<endl;
       remove();
       scaleToRangenum(newNotes, startTime, endTime, startIndexnum, endIndexnum, schrittwanz, firstrec);
       kk, kl, kr = false;
    }
}
        ...
        if (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.sym == SDLK_k) {
                    ...
                    cout << "Geben Sie den schrittwanz ein zwischen: "<<recorder1.Indexnumlaenge/(1/recorder1.startIndexnum)<<" und "<<recorder1.Indexnumlaenge/(1/recorder1.endIndexnum)<<endl;
                    recorder1.schrittwanz = stoi(recorder1.Eingabe());
                    recorder1.scaleToRangenum(recorder1.newNotes, recorder1.startTime, recorder1.endTime, recorder1.startIndexnum, recorder1.endIndexnum, recorder1.schrittwanz, recorder1.firstrec);
                    kpressed=false;
